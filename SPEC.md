...
7. **Aggregate "Business Overview" dashboard across all projects (missed earlier, capturing now).** Early on, the user compared this to the "Key Metrics" tab in her separate Google Sheets Business Tracker (Active Projects, Total Pipeline Budget, Total Invoiced All-Time, Total Received, Total Spent, Net Cash Position, etc.) — the intent was always to have something like this inside Studio Suite itself, not just per-project. This never got built or formally scoped; the existing Income & Expense tab only looks at the *current* project. This needs to be a real, separate page: sums Income & Expense (and Invoice/PO history) **across every project**, computed from Studio Suite's own database — **not pulled from Google Sheets**, consistent with the one-way sync architecture noted above (Studio Suite is the source of truth; a dashboard here should compute its own totals, not read them back from a mirror).

   **Also fold in a lightweight, flexible budget-allocation feature here, once this page exists:** rather than a single hardcoded "marketing %" line, build this as a small list of **user-defined percentage buckets** — each with a name (e.g. "Marketing," "Profit," "Tax Reserve," "Owner Pay") and a percentage of total income. The dashboard shows, per bucket: target amount (income × percentage) vs. actual amount spent/set aside (pulled from matching manual finance entries, matched by category name). User can add/edit/remove buckets freely as her real percentages get finalized (e.g. with a CPA) — this should not assume any specific categories or numbers up front, since she's still working those out. **Note:** QuickBooks Online (Plus/Advanced plans) already has a proper full-featured budgeting tool (by category, month/quarter/year, Budget vs. Actuals reporting) — if she wants a complete, formal business budget beyond these quick allocation ratios, that's likely a better fit there than building it here. This dashboard addition is meant to be a small, flexible convenience, not a competitor to QuickBooks' budgeting.
